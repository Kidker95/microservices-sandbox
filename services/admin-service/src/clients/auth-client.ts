...
class AuthClient {
    ...
    try {
        response = await this.fetchWithTimeout(`${this.authServiceBaseUrl}/api/auth/login`, {
            method: "POST",
    ...
    if (response.status === StatusCode.Unauthorized) throw new UnauthorizedError(data?.error || "Invalid credentials");
    if (response.status === StatusCode.Forbidden) throw new ForbiddenError(data?.error || "Forbidden");
    if (!response.ok) throw new BadRequestError(data?.error || `auth-service error: ${response.status} ${response.statusText}`);

    const token = data?.token;
    if (!token) throw new BadRequestError("auth-service did not return a token");
